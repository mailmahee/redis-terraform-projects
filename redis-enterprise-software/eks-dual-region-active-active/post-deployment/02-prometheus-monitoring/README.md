...
3. **Use generated YAML files** from `generated/region1/` or `generated/region2/`
4. **Create** the `monitoring` namespace
5. **Install** Prometheus Operator and CRDs from official bundle
6. **Deploy** Prometheus instance with cross-namespace discovery
7. **Deploy** Grafana (if `grafana_enabled=true`) with pre-configured Prometheus datasource
8. **Deploy** ServiceMonitor for automatic Redis Enterprise discovery
9. **Deploy** Prometheus alert rules for Redis health monitoring
