...
check_rerc_ready() {
    ...
    printf "kubectl get rerc $rerc_name -n $NAMESPACE --context $context"
...
try_api_request() {
    ...
    for endpoint in "${endpoints[@]}"; do
        if curl -ksSfu "$user:$pass" ${connect_args[@]+"${connect_args[@]}"} "$endpoint" -o "$response_file" --connect-timeout 10 --max-time 30; then
