...
#==============================================================================
# DNS OUTPUTS (if DNS module is enabled)
#==============================================================================

output "dns_records_created" {
  description = "Whether DNS records were automatically created"
  value       = length(module.dns) > 0 ? module.dns[0].dns_records_created : false
}

output "dns_api_record_fqdn" {
  description = "The FQDN of the created API DNS record"
  value       = length(module.dns) > 0 ? module.dns[0].api_record_fqdn : ""
}

output "dns_db_wildcard_record_fqdn" {
  description = "The wildcard FQDN of the created database DNS record"
  value       = length(module.dns) > 0 ? module.dns[0].db_wildcard_record_fqdn : ""
}

output "nginx_loadbalancer_dns" {
  description = "NGINX Ingress LoadBalancer DNS name"
  value       = var.external_access_type == "nginx-ingress" && length(module.external_access) > 0 ? module.external_access[0].nginx_ingress_dns : ""
}
